...
export class ProductListComponent implements OnInit, OnDestroy {
  ...
  ProductLists() {
    ...
    this.filterParams.upc = this.upc || '';
  ...
  receiveProgress(event) {
    ...
    this.offset = 0;
    if (this.keyword !== '' || this.sku !== '' || this.upc !== '' || this.status !== '' || this.price !== '') {
  ...
  product_Details(slug) {
    window.open(this.productUrl + 'product/' + slug);
  }
  ...
  exportAllExcel(){
    this.productSandbox.productAllExcel(null);
  ...
  exportAsXLSX(){
    console.log("prinyt")
    const payload:any=`status=${this.exportReportForm.value.orderStatus}`
this.productService.orderReportApi(payload).subscribe((res:any)=>{
  console.log('ressss',res)
if(res.length>0){
    this.excelService.exportAsExcelFile(res, 'product-report')
}   else{
