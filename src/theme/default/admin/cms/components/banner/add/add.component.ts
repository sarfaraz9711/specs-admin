...
export class BannerAddComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    if(sessionStorage.getItem("bannerAction")=="furo"){
      this.bannerFor='furo'
      }else {
        this.bannerFor='redchief'
    }
  ...
  bannerCancel() {
    ...
    if(sessionStorage.getItem("bannerAction")=="furo"){
      this.router.navigate(['/cms/furo-banners/list'], { queryParams: this.queryDetails });
      }else {
  ...
  public onSubmit() {
    ...
    if (!this.bannerForm.valid) {
      ...
      params.bannerFor = this.bannerForm.value.bannerFor || '';
  ...
  setBanner(details) {
    ...
    this.bannerForm.controls['bannerFor'].setValue(details.bannerFor);
