    ...
    switch (type) {
        ...
        case actions.ActionTypes.DO_blog_LIST_SUCCESS: {
            const blogListModel = payload.data.map(_bloglistModel => {
                if (_bloglistModel && _bloglistModel.description) {
                    _bloglistModel.description = _bloglistModel.description.replace(/<img[^>]*>/g, '');
                }
        ...
        case actions.ActionTypes.DO_CATEGORIES_LIST_SUCCESS: {
            let stateCategoryArrayList = [];
            stateCategoryArrayList = state.categoryList ? state.categoryList : payload.data;
            let concatArray = [];
            if (state.categoryList && !state.productRefresh) {
                concatArray = [...stateCategoryArrayList, ...payload.data];
            } else {
                concatArray = stateCategoryArrayList;
            }
            return Object.assign({}, state, {
                categoryList: concatArray,
...
export const getCategoryList = (state: BlogState) => state.categoryList;
