...
public productExportAll(){
  return this.http.get(this.basUrl + '/product/allproduct-excel-list');
}
  /**
 * Handles 'ProductExcel' function. Calls put method with specific api address
 * along its param.
 ...
 */
  ...
  orderReportApi(query:any){
    return this.http.get(this.basUrl + `/product/all-product-download?${query}`);
  }
  exportProduct(){
    return this.http.get(this.basUrl + `/product/all-product-download-list`);
  }
}
