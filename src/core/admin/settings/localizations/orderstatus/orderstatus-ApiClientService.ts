...
export class OrderstatusApiClientService extends Api {
  ...
  public updateProductTrackingstatus(param): Observable<any> {
    this.url = this.getBaseUrl();
    return this.http.post(
      this.url + '/order-tracking/secure/update-tracking',
      param
    );
  }
  ...
  public getUCstatus(id: any): Observable<any> {
    this.url = this.getBaseUrl();
    return this.http.get(this.url + `/order/uc-response-failed-order?orderId=${id}`)
  }
}
